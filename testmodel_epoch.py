...
# List of models saved for different epochs
epoch_numbers = [100, 200, 300, 500]
# List of epochs for which models are saved
results = {}

for epoch in epoch_numbers:
    # Update to the path where your models are saved
    print(f"\nEvaluating model trained for {epoch} epochs")
    test_loss, test_accuracy, precision, recall, f1, predicted_classes = evaluate_model(model, X_test, y_test)
    results[epoch] = {
        'test_loss': test_loss,
...
for epoch in epoch_numbers:
    np.save(f'predictions_epoch_{epoch}.npy', results[epoch]['predicted_classes'])
...
for i, epoch in enumerate(epoch_numbers):
    row = i // 2
    col = i % 2
    values = [results[epoch][metric] * 100 for metric in metrics]
